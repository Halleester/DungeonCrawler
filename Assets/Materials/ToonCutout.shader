Shader "Custom/ToonCutout" {
	...
	SubShader {
		...
		#pragma surface surf Stepped alphatest:_Cutoff fullforwardshadows
		#pragma target 3.0
		...
		float4 LightingStepped(ToonSurfaceOutput s, float3 lightDir, half3 viewDir, float shadowAttenuation){
			...
			float towardsLight = dot(s.Normal, lightDir) * shadowAttenuation;

            //float3 distanceLightColor = lerp(float3(1, 1, 1), _LightColor0.rgb, smoothstep(0, 1, towardsLight * _ColorDropoff));
            
            float3 distanceLightColor =  _LightColor0.rgb;

            //stretch values so each whole value is one step
            towardsLight = towardsLight / _StepWidth;

            //float normalAmt = fwidth(abs(dot(s.Normal, float3(0, 1, 0)) - 1) * 0.5);
            ...
            float smoothing = smoothstep(0, _RampSmoothness, frac(towardsLight));
            ...
            color.rgb = s.Albedo * lightIntensity * distanceLightColor;
